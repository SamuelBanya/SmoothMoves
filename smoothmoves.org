...
<2022-10-13 Thu 16:55>: 'react-step-wizard' Notes:
- I did more searching, and honestly, I wish I had known about this particular package called 'react-step-wizard' as this would have been a better option in this scenario, here are my related notes:
- https://www.npmjs.com/package/react-step-wizard
- This is the official example of the 'react-step-wizard':
- https://jcmcneal.github.io/react-step-wizard/app/
- This is another great live example of the 'react-step-wizard' from Codesandbox:
- https://codesandbox.io/s/j5mcq?file=/src/sample.js

<2022-10-13 Thu 21:05>: After reading that issues page for the unrelated 'react-final-form' library (https://github.com/final-form/react-final-form/issues/803), I observed a few hacky type solutions to the 'unmounted' wizard form issue I came across:

First Hacky Solution (Quote)
...
Second Hacky Solution (Quote)
- What we ended up actually doing was creating a fake Field component, which lived beside the accordion.
- It would watch for form changes, and set its own dirty field state to true when a change is made.
- So when an accordion item closes, this fake Field would never become unmounted, and therefore the overall form's dirty state won't get set back to false.

<2022-10-13 Thu 23:58>: I did more research on how to fix the issue of an unmounted component and found this StackOverflow post on the topic of using the 'isMounted' property value for the 'useEffect' hook:
- https://stackoverflow.com/questions/53949393/cant-perform-a-react-state-update-on-an-unmounted-component

<2022-10-14 Fri 00:28>: I tried even using React's Docs page on how to create a 'Basic List Component' but I still can't get anything to display properly:
- https://reactjs-bot.github.io/react/docs/lists-conditional-rendering.html

Based on the behavior of what's happening, the 'renderedChecklistItems' is getting it's values reset for some reason, which I have absolutely no idea why this is occurring.

<2022-10-14 Fri 01:06>: I took a deeper dive, and tried a few examples with console.log() based output, and realized that the only issue for 'newTotalItemsArray' is that the very first element is the one that doesn't have the most updated information for its value. I'm not sure if this is a side effect of the fact that its the first element, or the 'unmounting' phenomenon mentioned above.

However, at least its not happening for ALL forms after it. This is good to some extent so that I can narrow down the 'Why' this is occurring.

<2022-10-14 Fri 01:47>: I also narrowed it down to the fact that the 'totalItemsArray' is pretty much THE culprit in this scenario, which lives in the 'collectItemFormData' function in the parent ItemsForm component. For some reason, the first item is not being updated accordingly.

These are my other notes regarding this issue:
- ONLY the first item in the form returns a blank result until you go back and add a value to the controlled form, which needs to be fixed
- I think it's because of the very first time the 'useState' hook is called for 'itemFormData', it is adding the default 'zeroed' values above from 'itemFormData' to the beginning of the list
- I think this is something to do with how 'totalItemsArray' is being updated for the VERY first item

<2022-10-14 Fri 01:50>: To keep pressing slightly forward, I found a really good example to use as a basis for the checklist portion to display at the bottom of the page since this example uses Material UI as well:
- https://codesandbox.io/s/3jwge?file=/src/App.js

I did separate research and found the 'Checkbox' component from Material UI as well so I'll probably just use this by default in the most basic way possible:
- https://mui.com/material-ui/api/checkbox/

<2022-10-14 Fri 19:04>: This looks like a good reference for learning how to make asynchronous fetch requests as well:
- https://designcode.io/react-hooks-handbook-fetch-data-from-an-api

<2022-10-14 Fri 19:05>: I was able to fix the checklist rendering issue by literally using .map() outside of the fetch() requests that were previously there which resolved the issue.


<2022-10-14 Fri 19:29>: I figured out how to center the Material UI based 'Checklist' component by wrapping it inside a '<Box>' Material UI component via these reference links:
- https://mui.com/material-ui/api/box/
- https://stackoverflow.com/questions/60882578/how-to-vertical-align-material-ui-checkbox
- https://www.codegrepper.com/code-examples/whatever/box+align+self+center+material+ui

Related code snippet for the 'Box' section idea itself that I used:
#+begin_src js
<Box
display="flex"
alignItems="center"
...
* Overall Workflow Of App / Basis For Related Blog Post
<2022-10-14 Fri 19:41>: As I am complete with this app, I can easily say this has been quite a challenge to pull off. Not only is it using multiple React controlled forms, but it is using 'useState' and 'useEffect' hooks to make sure everything is in sync within certain parent and child components.
